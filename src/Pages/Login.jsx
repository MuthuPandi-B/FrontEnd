...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("https://backend-pmn1.onrender.com/api/auth/login", { email, password });
      toast.success("Login successful!");
      navigate("/dashboard"); // Redirect to dashboard or main page after login
    } catch (error) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md w-80">
        ...
        />
        <button type="submit" className="w-full p-3 bg-indigo-500 text-white rounded hover:bg-indigo-600 transition">
          Login
        </button>
        ...
        <p className="text-center text-sm text-gray-500 mt-2">
          ...
          <a href="/forgot-password" className="text-indigo-500 hover:underline">Reset it here</a>
        </p>
      </form>
    </div>
  );
